...
class ConfigEditor:
    def __init__(self, master):
        self.master = master
        self.master.title("PKUElcetive2025Autumn")
        root.state("zoomed")
        self.setup_style()
        ...
        # 配置区域框架
        self.config_frame = ttk.LabelFrame(main_frame, text="设置")
        ...
        # 保存按钮
        save_btn_frame = ttk.Frame(self.config_frame)
        save_btn_frame.pack(fill='x', pady=(0, 5))
        ttk.Button(save_btn_frame, text="保存配置",
                   command=self.save_config).pack(pady=5)

        # 选课输出部分
        # 控制台区域框架
        self.console_frame = ttk.LabelFrame(main_frame, text="日志信息")
        ...
        self.start_btn = ttk.Button(
            control_frame, text="启动", command=self.start_process, style='Accent.TButton')
        self.start_btn.pack(side='left', padx=5)

        self.stop_btn = ttk.Button(
            control_frame, text="停止", command=self.stop_process, state='disabled')
        self.stop_btn.pack(side='left', padx=5)

        # 控制台输出
        self.console = ScrolledText(self.console_frame, height=10, wrap='word')
    ...
    def setup_style(self):
        ...
        style.configure('Help.TLabelframe', background=bg_color,
                        bordercolor='#a0a0a0')
        style.configure('Help.TLabelframe.Label',
                        foreground='#606060', font=('Segoe UI', 9))
        style.configure('Help.TLabel', foreground='#404040',
                        background=bg_color)
        ...
        # 滚动条样式
        style.configure('TScrollbar', troughcolor=bg_color,
...
4. 最小间隔：数字，若消息产生时，距离上次成功发送不足这一时间，则取消发送。-1为不限制"""
    ...
    def start_process(self):
        ...
        # 添加开始说明
        self.append_output(
            "\n-----------------PKUElective2025Autumn-----------------\n原作者github链接：https://github.com/zhuozhiyongde/PKUElective2024Spring\n-->让人人都享受科技的便利！\n\n")
        ...
        # 启动外部程序
        exe_path = os.path.join(os.path.dirname(__file__), 'temp1.exe')
    ...
    def read_output(self):
        ...
        # 读取剩余输出
        if self.running:
            remaining = self.process.communicate()[0]
            if remaining:
    ...
    def show_config_area(self):
        """重新显示设置区域"""
        self.config_frame.pack(fill='both', expand=True,
                               pady=5, before=self.console_frame)
